						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable" id="data-berita">
						  ...
						  <tbody>
						  	<?php if ($data1->num_rows() == 0) { ?>
						  		<tr>
						  			<td colspan="5">Belum ada artikel</td>
						  		</tr>
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_artikel'] ?>">
										...
										<td class="center">
											...
											<a class="btn btn-info" href="<?php echo site_url('staf/edit_berita/'.$artikel['id_artikel']) ?>" title="Edit artikel" data-rel="tooltip">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'staf/hapus_artikel/' + idArt, {id_artikel: idArt, ajax: 1}, function(data) {
