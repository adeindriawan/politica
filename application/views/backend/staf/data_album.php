						  		...
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_album'] ?>">
										...
										<td class="center">
											...
											<a class="btn btn-info" href="<?php echo site_url('staf/edit-album/'.$artikel['id_album']) ?>" title="Edit album" data-rel="tooltip">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'staf/hapus_album/' + idArt, {id_album: idArt, ajax: 1}, function(data) {
