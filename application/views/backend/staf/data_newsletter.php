						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_newsletter'] ?>">
										...
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('staf/detail-newsletter/'.$artikel['id_newsletter']) ?>" title="Lihat newsletter" data-rel="tooltip">
											...
											<?php if ($artikel['status_newsletter'] == 'Created') { ?>
												...
												<a class="btn btn-info" href="<?php echo site_url('staf/edit-newsletter/'.$artikel['id_newsletter']) ?>" title="Edit newsletter" data-rel="tooltip">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actKirimNewsletter').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$('.loadingImage').show();
			$.post(link + 'staf/kirim_newsletter/' + idArt, {id_newsletter: idArt, ajax: 1}, function(data) {
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'staf/hapus_newsletter/' + idArt, {id_newsletter: idArt, ajax: 1}, function(data) {
				/*optional stuff to do after success */
				if (data == "true") {
