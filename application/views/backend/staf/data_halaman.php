						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_halaman'] ?>">
										...
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('staf/detail-halaman/'.$artikel['id_halaman']) ?>" title="Lihat halaman" data-rel="tooltip">
												<i class="halflings-icon white zoom-in"></i>  
											</a>
											<a class="btn btn-info" href="<?php echo site_url('staf/edit-halaman/'.$artikel['id_halaman']) ?>" title="Edit halaman" data-rel="tooltip">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'staf/hapus_halaman/' + idArt, {id_artikel: idArt, ajax: 1}, function(data) {
