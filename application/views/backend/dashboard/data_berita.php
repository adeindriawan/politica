			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon user"></i><span class="break"></span>Data Berita (Ada <?php echo $data3 ?> artikel berita)</h2>
						<div class="box-icon">
						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_artikel'] ?>">
										...
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('berita/artikel/'.$artikel['id_artikel']) ?>" title="Lihat artikel" data-rel="tooltip">
												<i class="halflings-icon white zoom-in"></i>  
											</a>
											<a class="btn btn-info" href="<?php echo site_url('dashboard/detail-berita/'.$artikel['id_artikel']) ?>" title="Edit artikel" data-rel="tooltip">
												<i class="halflings-icon white edit"></i>  
											</a>
											<?php if (!is_null($this->session->userdata('bisa_hapus_berita'))) { ?>
												<a class="btn btn-danger" href="#" id="tmblHapus<?php echo $artikel['id_artikel'] ?>" value="<?php echo $artikel['id_artikel'] ?>" judul="<?php echo $artikel['judul_artikel'] ?>" title="Hapus artikel" data-rel="tooltip">
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'dashboard/hapus_berita/' + idArt, {id_artikel: idArt, ajax: 1}, function(data) {
