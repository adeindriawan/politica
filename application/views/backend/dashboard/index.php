...
<div class="clearfix"></div>
<?php if ($this->session->flashdata('akses_ditolak')) { ?>
	<div class="modal hide fade" id="errorValModal">
		...
		<div class="modal-body tambah-tag">
			<?php echo $this->session->flashdata('akses_ditolak'); ?>
		</div>
...
<script type="text/javascript">
    $(document).ready(function() {
      ...
      function cek_notifikasi_lagi (id) {
        $.post(link + 'dashboard/cek_notifikasi', {id_notifikasi: id, session_id: session_id, ajax: 1}, function(data, textStatus, xhr) {
            ...
            if (response.length == 0) {
              ...
              $(function(){
                $.each(response, function(index, val) {
                   ...
                   if (val.username == null) { val.username_admin = "(User tidak log in)"} else{val.username};
                   $.gritter.add({
                    title: "Notifikasi",
      ...
      $.post(link + 'dashboard/cek_notifikasi', {session_id: session_id, ajax: 1}, function(data) {
        /*optional stuff to do after success */
        if (data == "false") {
          $.gritter.add({
            title: "Notifikasi",
...
function charts() {
	...
	{	
		...
		$.post(link + 'dashboard/pageviews_and_visit_chart', {ajax: 1}, function(data) {
			/*optional stuff to do after success */
			if (data == 'false') {
				$('#stats-chart2').hide();
			} else {
