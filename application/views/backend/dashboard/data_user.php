						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $element) { ?>
							  		<tr id="tr<?php echo $element['id_admin'] ?>">
										...
										<td class="center"><?php echo $element['email_admin'] ?></td>
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('dashboard/detail-user/'.$element['id_admin']) ?>" title="Lihat profil" data-rel="tooltip">
												<i class="halflings-icon white zoom-in"></i>  
											</a>
											<?php if (!is_null($this->session->userdata('bisa_ubah_user'))) { ?>
												<a class="btn btn-info" href="<?php echo site_url('dashboard/edit-user/'.$element['id_admin']) ?>" title="Edit user" data-rel="tooltip">
													<i class="halflings-icon white edit"></i>  
												</a>
											<?php } ?>
											<?php if (!is_null($this->session->userdata('bisa_hapus_user'))) { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'dashboard/hapus_user/' + idArt, {id_admin: idArt, ajax: 1}, function(data) {
