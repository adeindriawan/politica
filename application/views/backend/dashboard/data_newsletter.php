						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_newsletter'] ?>">
										...
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('dashboard/detail-newsletter/'.$artikel['id_newsletter']) ?>" title="Lihat newsletter" data-rel="tooltip">
											...
											<?php if ($artikel['status_newsletter'] == 'Created') { ?>
												...
												<?php if (!is_null($this->session->userdata('bisa_hapus_newsletter'))) { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actKirimNewsletter').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$('.loadingImage').show();
			$.post(link + 'dashboard/kirim_newsletter/' + idArt, {id_newsletter: idArt, ajax: 1}, function(data) {
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'dashboard/hapus_newsletter/' + idArt, {id_newsletter: idArt, ajax: 1}, function(data) {
				/*optional stuff to do after success */
				if (data == "true") {
