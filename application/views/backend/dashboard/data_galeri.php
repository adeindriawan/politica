						  	...
						  	<?php } else { ?>
						  		<?php foreach ($data1->result_array() as $artikel) { ?>
							  		<tr id="tr<?php echo $artikel['id_album'] ?>">
										...
										<td class="center">
											<a class="btn btn-success" href="<?php echo site_url('galeri/album/'.$artikel['id_album']) ?>" title="Lihat album" data-rel="tooltip">
												<i class="halflings-icon white zoom-in"></i>  
											</a>
											<a class="btn btn-info" href="<?php echo site_url('dashboard/detail-galeri/'.$artikel['id_album']) ?>" title="Edit album" data-rel="tooltip">
												<i class="halflings-icon white edit"></i>  
											</a>
											<?php if (!is_null($this->session->userdata('bisa_hapus_galeri'))) { ?>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#actHapusArtikel').click(function(event) {
			...
			var link = "<?php echo base_url() ?>";
			$.post(link + 'dashboard/hapus_galeri/' + idArt, {id_album: idArt, ajax: 1}, function(data) {
